...
/**
 * Queue worker (no HTTP port). Hosts the BullMQ consumers for scan + extraction + saft-export.
...
async function bootstrap(): Promise<void> {
  ...
  log.log(`Worker up — ${workers.length} queue consumers bound (scan, extraction, saft-export).`);
