...
import { buildHeader, monthBounds, derivePurchaseAmounts } from '../domain/assemble';
...
export class SaftDatasetBuilder implements ISaftDatasetBuilder {
  ...
  /**
   * A purchase source document is a non-reversal review entry that has NOT itself been reversed
   * (Phase 8 #7): exclude both the reversal entry and the original it cancels.
   */
  private isPurchaseEntry(e: SaftGlEntry, reversedIds: Set<string>): boolean {
    return e.sourceType === 'review' && !e.reversesEntryId && !!e.sourceId && !reversedIds.has(e.journalEntryId);
  }
  ...
  async buildDataset(year: number, month: number): Promise<SaftDataset> {
    ...
    const reversedIds = new Set(reads.generalLedgerEntries.filter((e) => e.reversesEntryId).map((e) => e.reversesEntryId as string));
    const purchaseEntries = reads.generalLedgerEntries.filter((e) => this.isPurchaseEntry(e, reversedIds));
    ...
    const purchaseDocuments: SaftPurchaseDocument[] = purchaseEntries.map((e) => {
      ...
      // gross from the posted lines (mapping-free); net/VAT from the approved document when it reconciles.
      const amounts = derivePurchaseAmounts(e.lines, { net: d?.documentNet, vat: d?.documentVat, gross: d?.documentGross });
      return {
        supplier: d?.supplierName, supplierId: d?.supplierId,
        documentNumber: d?.documentNumber ?? `#${e.entryNo}`, documentDate: e.postingDate,
