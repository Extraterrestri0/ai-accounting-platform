-- =====================================================================
-- 0016 Self-service registration + OAuth account provisioning.
-- app_user has only SELECT on tenants/users/organizations, so signup must
-- run through SECURITY DEFINER functions (same pattern as authenticate_lookup):
-- they create a tenant + user + tenant_admin membership + an initial company,
-- atomically, then app_user logs in normally. RLS/FORCE remain untouched.
-- =====================================================================

-- Password registration: fails if the email already exists.
...
-- OAuth provisioning: returns the existing user (login) or creates a new one (no password).
